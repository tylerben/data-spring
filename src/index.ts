 ...
 */
const calcTotalTicks = (start: string, end: string, intervalBy: Intervals) => {
...
export const generateData = (config: iConfig[]) => {
  ...
  if (dateField) {
    ...
    const maxDate = dateField.min as string;
    const interval = dateField.interval;

    const length = calcTotalTicks(
      minDate as string,
      maxDate! as string,
      interval!
    );
